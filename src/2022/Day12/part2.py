...
for j, row in enumerate(the_map):
    ...
    for i, height in enumerate(row):
        height_num = char_to_int(height)
        the_map_num[-1].append(height_num)
        ...
        for heap_index, (heap_d, heap_j, heap_i) in enumerate(squares_heap):
            if (heap_j, heap_i) == (next_j, next_i):
                ...
                if distance + 1 < heap_d:
                    ...
                    if the_map_num[next_j][next_i] == 0:
                        final_distances[(next_j, next_i)] = distance + 1
                break

print(min(final_distances.values()))