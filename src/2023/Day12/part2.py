...
def get_arrangements(record_text, record_num):
    ...
    if (record_text, tuple(record_num)) in seen_arrangements:
        arrangements = seen_arrangements[(record_text, tuple(record_num))]
        return arrangements
    ...
    while not done:
        ...
        while record_text[0] == "#":
            ...
            if len(record_text) == record_num[0] and len(record_num) == 1:
                arrangements = 1
                seen_arrangements[(record_text, tuple(record_num))] = arrangements
                return arrangements
...
for i, input_line in enumerate(input_lines):
    ...
    record_text_input = record_text
    record_num_input = record_num.copy()

    # unfold
    for _ in range(4):
        record_text = record_text + "?" + record_text_input
        record_num.extend(record_num_input)
